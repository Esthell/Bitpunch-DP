...
/**
 * Create a binary vector of required length, weight and with distance d of all ones in vector. 
 *  After work call BPU_freeVecGF2
 * @param  out GF2 vector representation (L)
 * @param  t number of ones
 * @param  d distance of every two ones in vector
 * @return     0 - succes, else error
 */
int BPU_gf2VecWithDist(BPU_T_GF2_Vector * out, int t, int d);

/**
 * Return distances of every two ones in row h0 of matrix H saved in out
 * @param  in - row h0 of matrix H
 * @param  out
 * @param  m - modulus
 * @return  0
 */
...
/**
 * Return multiplicity of distances and array of counters
 * @param  in - distances 
 * @param  out  - empty
 * @param  numDist - number of distances
 * @param  maxDistance - max possible Lee Distance
 * @param  length - number of rows in multiplicity
 * @return  0;
 */
