...
void BPU_gf2SparseQcMatrixMalloc(BPU_T_GF2_Sparse_Qc_Matrix * v,
                                 uint16_t element_count, uint32_t element_size,
                                 uint8_t isVertical) {
    // allocate matrices
    v->matrices =
...
int BPU_gf2VecWithDist(BPU_T_GF2_Vector * out, int t, int d) {
    int i;
    int length = out->len / 2;
    uint32_t rand;
   
//    rand = BPU_prngGetRand(0,2);
//    printf("%d \n",rand);
    
//    printf("d = %d \n",d);
    
    if (t > out->len) {
        BPU_printError("weight error t > l");
        return -2;
    }
    
    if (t == 0) {
        BPU_printError("weight error t == 0");
        return -2;
    }
    ...
    //vector of required weight
        ...
        for (i = 0; i < t/2; ){
            rand = BPU_prngGetRand(0,length);
//            printf("%d \n",rand);
            if(BPU_gf2VecGetBit(out, rand) != 1 && BPU_gf2VecGetBit(out,(rand + d) % length) != 1){                              
                BPU_gf2VecSetBit(out, rand, 1);
                BPU_gf2VecSetBit(out, (rand + d) % length,1 );
                i++;
            }                        
        }
...
int BPU_gf2SparseGetDist(BPU_T_GF2_Sparse_Poly * in, uint16_t * out, int m) {
   
    int i,j,counter = 0;
    
    for (i = 0; i < in->weight; i++){
        for (j = i+1; j < in->weight; j++){
            out[counter] = min(absVal(in->index[i] - in->index[j]),m - absVal(in->index[i] - in->index[j]));
            counter++;
        }
    }
...
int BPU_gf2GetMultiplicity(uint16_t * in, int inLen, int ** out, int numDist, int maxDistance, int length){
    
    int i,j;
  
    uint16_t **tmp;
    uint16_t ** multi = (uint16_t **)calloc(length,sizeof(uint16_t)); 
    uint16_t * counter = (uint16_t *)calloc(length,sizeof(uint16_t));
           
    for (i = 0; i < length; i++){ 
        counter[i] = 0;
    }    
    
    tmp = (uint16_t **)calloc(maxDistance,sizeof(uint16_t *)); 
   
    for (i = 0; i < maxDistance; i++){ 
        tmp[i] = (uint16_t *)calloc(1,sizeof(uint16_t));  
        tmp[i][0] = 0;
    }              
    ...
    for(i = 0; i < length; i++){
        multi[i] = (uint16_t *)calloc(sizeof(uint16_t),1); 
    }
    
    for(i = 1; i < (maxDistance+1); i++){
        switch(tmp[i-1][0]){
            case 0:
//                printf("0: %d \n",i);
                multi[0] = (uint16_t *)realloc(multi[0], sizeof(uint16_t)*(counter[0]+1)); 
                multi[0][counter[0]] = i;
                counter[0]++;
                break;
            case 1:           
//                printf("1: %d \n",i);
                multi[1] = (uint16_t *)realloc(multi[1], sizeof(uint16_t)*(counter[1]+1)); 
                multi[1][counter[1]] = i;
                counter[1]++;
                break;
            case 2:             
//                printf("2: %d \n",i);
                multi[2] = (uint16_t *)realloc(multi[2], sizeof(uint16_t)*(counter[2]+1)); 
                multi[2][counter[2]] = i;
                counter[2]++;
                break;
            case 3:
//                printf("3: %d \n",i);
                multi[3] = (uint16_t *)realloc(multi[3], sizeof(uint16_t)*(counter[3]+1)); 
                multi[3][counter[3]] = i;
                counter[3]++;
                break;          
            default:
...
void BPU_gf2SparseQcMatrixTranspForRealThisTime(BPU_T_GF2_Sparse_Qc_Matrix * out,
                                 const BPU_T_GF2_Sparse_Qc_Matrix * in) {
    int counter, coeff, ele, zero_coeff_is_set;

    // allocate memory for output matrix
    BPU_gf2SparseQcMatrixMalloc(out, in->element_count, in->element_size, !in->isVertical);
    ...
    if (row_num < m->k) {
        ...
        if (m->isVertical){
            ...
            for (int j = 0; j < m->element_count; ++j) {
                weight += m->matrices[j].weight;
            }
            fprintf(stderr, "weight during get row: %d\n", weight);
            BPU_gf2SparsePolyMalloc(p, weight);
            uint32_t offset = 0;
            for(int j = 0; j < m->element_count; j++){
                for (int k = 0; k < m->matrices[j].weight; k++) {
                    p->index[k+offset] = m->matrices[j].index[k];
                }
                for (int k = 0; k < p->weight; k++){
                    // shift coefficients
                    p->index[k+offset] = (((p->index[k+offset]) + row_num) % m->element_size) + j*m->element_size;
                }
                offset += m->matrices[j].weight;
                fprintf(stderr, "offset during get row(after increment): %d\n", offset);
            }
...
void BPU_gf2SparseMatrixGetRow(BPU_T_GF2_Sparse_Poly * p,
                                 const BPU_T_GF2_Matrix* m,
                                 int weight,int rowLength,int row_num) {
    int i,count = 0;
    BPU_T_GF2_Vector *tmpRow;

    // if rownum is in matrix
    if (row_num < m->k) {
        // allocate output polynom
        BPU_gf2SparsePolyMalloc(p,weight);
        BPU_gf2VecMalloc(&tmpRow,rowLength);
        BPU_gf2MatGetRowAsGf2Vec(tmpRow,m, row_num);
        p->weight = weight;
        for(i = 0; i < rowLength; i++){
            if(BPU_gf2VecGetBit(tmpRow,i)){
                p->index[count] = i;
                count++;
            }            
        }
        BPU_gf2VecFree(&tmpRow);
            // for all coefficients
            /*for (i = 0; i < p->weight; i++){
                // shift coefficients 
                p->index[i] = ((p->index[i]) + row_num) % m->n;
            }*/
    }
    // row num is out of the matrix
    else {
        BPU_printError
            ("BPU_MatrixGetRow: row with index %i does not exist\n", row_num);
    }
}
