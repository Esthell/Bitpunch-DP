...
int BPU_mecsQcmdpcDecrypt(BPU_T_GF2_Vector * out, const BPU_T_GF2_Vector * in,
    ...
    switch(ctx->decoder_id){
        case 0:
            if (!BPU_mecsQcmdpcDecodeE_2(plainTextDecode, in, ctx)) {
            ...
            break;
        case 1:
            if (!BPU_mecsQcmdpcDecodeREMP1_2(plainTextDecode, in, ctx)) {
            ...
            break;
        case 2:
            if (!BPU_mecsQcmdpcDecodeREMP2_2(plainTextDecode, in, ctx)) {
...
int BPU_mecsQcmdpcDecodeE_2(BPU_T_GF2_Vector * plainTextVector,
    ...
    int retval = 1;
    int omega = 13;
    // struct _BPU_T_Code_Ctx *myctx = ctx;
    BPU_T_GF2_Poly syndrom;
    uint16_t w = ctx->code_spec->qcmdpc->w;
    uint32_t k = ctx->code_spec->qcmdpc->H.k; // 9602
    uint32_t n = ctx->code_spec->qcmdpc->H.n; // 4801
    ...
    for (int iter = 0; iter < BPU_QCMDPC_PARAM_MAX_ITER_C; ++iter) {
        ...
        BPU_mecsQcmdpcCalcSyndrom(&syndrom,plainTextVector,ctx);
        // myctx->syndrome_weight = BPU_gf2PolyWeight(&syndrom);
        if(BPU_gf2PolyIsZero(&syndrom)){
            retval = 0;
            break;
        }
